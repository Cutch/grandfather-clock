...
AWSResponse ESPAWSClient::doGet(String uri, String queryString, Task &t) {
  ...
  return send(request, t);
}

AWSResponse ESPAWSClient::doPost(String uri, String payload, String contentType, String queryString, Task &t) {
  String request = createRequest("POST", uri, payload, contentType, queryString);
  return send(request, t);
}

AWSResponse ESPAWSClient::send(const String request, Task *t) {
  ...
  if (connect(FQDN().c_str(), 443)) {
    ...
    print(request);
    // Read headers
    while(t != NULL && connected() && peek() < 0) (*t)();
    while (connected()) {
